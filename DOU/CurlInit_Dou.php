<?php
include_once '../abstractClass/CurlInit.php';
...
class CurlInit_Dou extends CurlInit
{
    protected function curlInit1($url, $nextNumberOfVacation = false)
    {
        if ($curl = curl_init()) {
            ...
            if ($nextNumberOfVacation == false) {
                curl_setopt($curl, CURLOPT_POSTFIELDS, "count=0&csrfmiddlewaretoken=sTZxFTpI7xU7TtxB4lVfNUTQcT55BFPm");
            } else {
                curl_setopt($curl, CURLOPT_POSTFIELDS, "count=$nextNumberOfVacation&csrfmiddlewaretoken=sTZxFTpI7xU7TtxB4lVfNUTQcT55BFPm");
            }
